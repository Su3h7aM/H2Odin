...
It is worth being honest that Transformation is not *mathematically* pure: a Lua policy can, in principle, do arbitrary work. So the determinism we care about is really a *convention the configuration is expected to honor* — well-behaved configuration is side-effect-free — rather than something the architecture forces. We document that expectation rather than pretend it is guaranteed. Analysis, which consults no policy, is deterministic without any such caveat.
