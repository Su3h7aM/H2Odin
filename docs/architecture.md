...
It is worth being honest that Transformation is not *mathematically* pure in the abstract: a Lua policy could still encode nondeterminism if it had a source of entropy. The sandbox removes the practical ones — `math.random` / `math.randomseed` are withheld with the raw loaders, and host side effects are blocked by withholding `io`, `os`, and `debug` — so output is deterministic for the same headers plus the same config tree. Once `require` exists a config may read Lua files beneath its own directory, so "no filesystem access" is a scoped guarantee rather than an absolute one. Analysis, which consults no policy, is deterministic without any such caveat.
