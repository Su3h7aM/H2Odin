...
**Why `known_tokens`.** Converting a C identifier to Odin case requires splitting it into words, and that split is not deterministic in general — `sqlite3` could be one token or `sqlite` + `3`; `HTTPSConnection` could split several ways. The generator applies a fixed heuristic and marks uncertain splits, but the reliable fix is letting the user declare their domain's vocabulary once. `known_tokens` upgrades those splits from *guessed* to *certain*: the user teaches the tokenizer that `SQLite3` is one atom with a known lowercasing, and every identifier containing it is handled correctly. Where the dictionary admits competing segmentations (longest match vs a shorter known key plus another known continuation — e.g. `ABC` as `AB|…` vs `A|BC`), the generator emits `naming_ambiguity` and the user resolves that one symbol via `override`. Three rungs — heuristic, dictionary, per-symbol override — each climbed only when the previous is insufficient.
...
`pointer = "multi"`, `require_results`, idiomatic `by_ptr = true`, and
`procs.wrappers` (out-param results + input slices) are implemented
(array-parameter decay also selects `[^]T`; block-level `@(require_results)`
when every proc in the foreign block is listed).
